...
from utils.Dataset import DataIterator
...
parser.add_argument('--corpus_tag_or_dir', type=str, default='./data/')
    ...
    for train_step in range(total_train_steps):
        ...
        if eval_iter is not None:
            if train_step > 0 and train_step % eval_per_n_steps == 0 or train_step == total_train_steps-1:
                ...
                logging.info(f'{int(train_step / eval_per_n_steps)}th epoch acc score is {acc}')
                # 将acc记录到summaries_writer
                summaries_writer.add_scalar('eval acc', acc, int(train_step / eval_per_n_steps))
                logging.info(f'{int(train_step / eval_per_n_steps)}th model evaluation is completed')
                # 如果当前验证集的acc比best_f1要大，则进行模型保存，并更新best_f1
                if acc > best_acc:
                    best_acc = acc
